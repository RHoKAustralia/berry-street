...
import RelationshipForm from './RelationshipForm.jsx'
...
export default class CaseRelationship extends Component {
  constructor (prop) {
    super(prop)
    this.state = {
      init: false,
  ...
  render () {
    if (!this.state.init) {
      ...
      if (this.state.relData) {
        ...
        return <div>
          <PersonForm person={this.state.person} handleSubmit={this.saveRelationship} />
          <RelationshipForm initVals={data} handleSubmit={this.saveRelationship} />
        </div>
      } else {
        return <div className="alert alert-info">
